...
qdrant_manager = HistoryQdrantManager("history_russia_clean")
...
class RAGService:
    ...
    async def retrieve(self, query: RAGQuery) -> RAGResult:
        ...
        question = query.query[:400].strip()
        ...
        if result == "НИЧЕГО НЕ НАШЕЛ НЕ НАДО ТЫКАТЬ ПО API ЛИШНИЙ РАЗ НЕЙРОНКУ!!!":
            ...
            documents = [RAGDocument(content=chunk, metadata=metadata) for chunk, metadata in result]
